...
function listarOrcamentos(){
    ...
    // LEFT JOIN garante a exibição de TODOS os orçamentos, tratando campos nulos
    $sql = "SELECT o.idOrcamento, 
                   COALESCE(c.nomeCliente, 'Cliente não encontrado') AS nomeCliente, 
                   COALESCE(m.nomeMarca, 'S/ Marca') AS nomeMarca, 
                   COALESCE(mo.nomeModelo, 'S/ Modelo') AS nomeModelo,  
            ...
            LEFT JOIN aparelho a ON o.Aparelho_idAparelho = a.idAparelho
    ...
    if($result && mysqli_num_rows($result) > 0){
        foreach($result as $coluna){
            ...
            // Tratamento do nome do aparelho concatenado
            $nomeAparelho = trim($coluna['nomeMarca'] . " " . $coluna['nomeModelo']);
            ...
            $html .= "<tr class='orcamento-row' style='cursor: pointer;'
                        ...
                        data-aparelho='".htmlspecialchars($nomeAparelho, ENT_QUOTES, 'UTF-8')."'
                    ...
                    <td>".htmlspecialchars($coluna['nomeCliente'], ENT_QUOTES, 'UTF-8')."</td>
                    <td>".htmlspecialchars($nomeAparelho, ENT_QUOTES, 'UTF-8')."</td>
                    <td>".htmlspecialchars($txtPecas, ENT_QUOTES, 'UTF-8')."</td>
